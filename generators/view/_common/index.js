...
module.exports = Generator.extend({
  ...
  addNpmDependencies: function (deps) {
    const pkg = _.merge(this._readPkg(), {
      dependencies: deps
    });
  ...
  addBowerDependencies: function (deps) {
    const bower = _.merge(this._readBower(), {
      dependencies: deps
    });
  ...
  addScripts: function () {
    ...
    const file = path.join('scripts', 'sw', 'runtime-caching.js');
    this.template(file, path.join(this.props.base, file));

    // Testfiles
    const dest = 'tests/Frontend';
  ...
  addFonts: function () {
    const dest = this.destinationPath('app/Resources/public/fonts');
    fs.mkdirsSync(dest);

    const src = this.destinationPath(this.props.noBower ? 'node_modules' : 'bower_components');
